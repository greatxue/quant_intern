...
class Alpha101(object):
    ...
    def alpha002(self):
        ...
        return df.replace([-np.inf, np.inf], 0).fillna(value=0)    # replace -inf, inf and NaN with 0
    ...
    def alpha021(self):
        ...
        #alpha = pd.DataFrame(np.ones_like(self.close), index=self.close.index, 
        #             columns=self.close.columns)    ## necessary for multiple-stock process
    ...
    def alpha060(self):
        divisor = (self.high - self.low).replace(0, 0.0001)    # replace all 0 as a minimized number in case of zero denominator
    ...
    def alpha064(self):
        ...
        return ((rank(correlation(sma(((self.open * 0.178404) + (self.low * (1 - 0.178404))), 13),sma(adv120, 13), 17)) < rank(delta(((((self.high + self.low) / 2) * 0.178404) + (self.vwap * (1 -0.178404))), int(3.69741)))) * -1)
    ...
    def alpha068(self):
        ...
        return ((ts_rank(correlation(rank(self.high), rank(adv15), 9), 14) <rank(delta(((self.close * 0.518371) + (self.low * (1 - 0.518371))), int(1.06157)))) * -1)
